...
class Qwen35:
    """Qwen 3.5 response activation measurement."""
    ...
    def project_response(
        ...
        *,
        ...
        hidden = self.response_hidden(messages, response, layers=[layer], pooling=pooling)[layer]
        user, assistant = load_role_vectors(vector_root, layer)
        direction = user - assistant
        direction /= torch.linalg.vector_norm(direction).clamp_min(1e-12)
        if centered:
            hidden = hidden - 0.5 * (user + assistant)
        return float(hidden @ direction)
